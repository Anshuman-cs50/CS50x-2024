...
bool cycle(int target, int head);
...
void record_preferences(int ranks[])
{
    ...
    // loop through ranks[]
    {
        ...
        // loop through every other ranks
        {
...
void add_pairs(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = i + 1; j < candidate_count; j++) // Start from i+1 because all values at (i = j) will be 0
        {
            // Check if there's a clear preference for one candidate over the other
...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        {
            // Unlock the pair if it creates a cycle
